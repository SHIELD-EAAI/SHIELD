...
class Parser:
    ...
    def parse_directory(self, source: str):
        ...
        print(f'file being processed {data_dir}')
    ...
    def map_events(self, source: str, timesplit: Optional[str] = None):
        ...
        self.logger.info ("Converting the files to pickle")
...
def main():
    ...
    parser.add_argument('--config', type=str, default='parser_config.yaml',
    ...
    ds_parser = Parser(args.config)
    
    if args.mode in ('parse', 'both'):
        ds_parser.parse_directory(args.source)
    
    if args.mode in ('map', 'both'):
        ds_parser.map_events(args.source, args.timesplit)
