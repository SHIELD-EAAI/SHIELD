...
class GraphAnalyzer:
    ...
    def analyze(self, df):
        ...
        filtered_df = self._create_and_filter_graph(df_suspicious, df)
        return filtered_df
    ...
    def _update_suspicious_files(self):
        for file_name, operations in self.file_operations.items():
            ...
            outward_processes = set(op[2] for op in operations if op[1] in ['write', 'modify'])
    ...
    def _has_suspicious_operations(self, operations):
        ...
        for _, event, process_uuid in operations:
            if event in ['read', 'write', 'execute', 'modify']:
                if last_operation:
    ...
    def _filter_dataframe(self, df):
        suspicious_files_df = df[(df['objectType'] == 'file') & (df['objectData'].isin(self.suspicious_files))]
    ...
    def _create_and_filter_graph(self, df_suspicious, original_df):
        ...
        for index, row in df_suspicious.iterrows():
            ...
            if object_data in ['/dev/pts/2', '<unknown>']:
                continue
